...
import 'package:quiz_app/class2/question23.dart';

class Question22Class2 extends StatefulWidget {
  const Question22Class2({super.key});

  @override
  State<Question22Class2> createState() => _Question22Class2State();
}

class _Question22Class2State extends State<Question22Class2> {
      ...
      body: Container(
        ...
        child: Column(
          children: [
            ...
            Image.asset('assets/images/class2/seta_img_soal_no_22.png'),
            const SizedBox(
              height: 5,
            ),
            const Align(
              alignment: Alignment.centerLeft,
              child: Text(
                'Selisih antara berat anggur dan berat apel adalah. ... gram',
                style: TextStyle(
                  fontSize: 14,
                  color: Color(0xff006699),
                ),
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => const Question23Class2()));
                    },
  ...
  openPlayer() async {
    await _assetAudioPlayer.open(
      Audio('assets/audios/class2/item22.mp3'),
      autoStart: true,
    );
    ...
    _assetAudioPlayer.currentPosition.listen((event) {
      setState(() {
        if (17290 <= event.inMilliseconds && event.inMilliseconds <= 18500) {
          isVisibleAnswerA = true;
        } else if (19712 <= event.inMilliseconds &&
            event.inMilliseconds <= 20500) {
          isVisibleAnswerB = true;
        } else if (21831 <= event.inMilliseconds &&
            event.inMilliseconds <= 22500) {
          isVisibleAnswerC = true;
        } else if (23924 <= event.inMilliseconds &&
