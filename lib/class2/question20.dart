...
import 'package:quiz_app/class2/question21.dart';

class Question20Class2 extends StatefulWidget {
  const Question20Class2({super.key});

  @override
  State<Question20Class2> createState() => _Question20Class2State();
}

class _Question20Class2State extends State<Question20Class2> {
      ...
      body: Container(
        ...
        child: Column(
          children: [
            ...
            Image.asset('assets/images/class2/seta_img_soal_no_20.png'),
            const SizedBox(
              height: 5,
            ),
            const Align(
              alignment: Alignment.centerLeft,
              child: Text(
                'Jarak dari Rumah Ami ke Rumah Sakit Sentosa adalah sejauh ...  kilometer',
                style: TextStyle(
                  fontSize: 14,
                  color: Color(0xff006699),
                ),
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => const Question21Class2()));
                    },
  ...
  openPlayer() async {
    await _assetAudioPlayer.open(
      Audio('assets/audios/class2/item20.mp3'),
      autoStart: true,
    );
    ...
    _assetAudioPlayer.currentPosition.listen((event) {
      setState(() {
        if (23008 <= event.inMilliseconds && event.inMilliseconds <= 24500) {
          isVisibleAnswerA = true;
        } else if (25166 <= event.inMilliseconds &&
            event.inMilliseconds <= 26500) {
          isVisibleAnswerB = true;
        } else if (27274 <= event.inMilliseconds &&
            event.inMilliseconds <= 28500) {
          isVisibleAnswerC = true;
        } else if (29351 <= event.inMilliseconds &&
