...
class _Question4Class2State extends State<Question4Class2> {
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) => const Question5Class2()));
                    },
  ...
  openPlayer() async {
    await _assetAudioPlayer.open(
      Audio('assets/audios/class2/item4.mp3'),
      autoStart: true,
    );
    ...
    _assetAudioPlayer.currentPosition.listen((event) {
      setState(() {
        if (20032 <= event.inMilliseconds && event.inMilliseconds <= 21500) {
          isVisibleAnswerA = true;
        } else if (22756 <= event.inMilliseconds &&
            event.inMilliseconds <= 23500) {
          isVisibleAnswerB = true;
        } else if (25187 <= event.inMilliseconds &&
            event.inMilliseconds <= 26500) {
          isVisibleAnswerC = true;
        } else if (27582 <= event.inMilliseconds &&
